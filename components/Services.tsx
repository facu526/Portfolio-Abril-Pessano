...
function MobileSafeVideoExamples() {
  return (
    <>
      <ExamplesHeader />
    ...
    <>
      ...
      <div className="mt-12 grid gap-6">
        {videoCategories.map((category) => (
          ...
          >
            <div className="relative z-10">
              <div className="flex items-end justify-between gap-5 text-left">
                <div>
                  <span className="rounded-full bg-paper/[0.72] px-3 py-1.5 text-xs font-black uppercase text-berry/[0.68] shadow-[0_10px_28px_rgba(168,72,115,0.08)]">
                    categoría
                  </span>
                  <h3 className="mt-4 text-4xl font-black text-ink">
                    {category.title}
                  </h3>
                </div>
                <span
                  className="h-3 w-56 rounded-full bg-gradient-to-r from-blush via-peach to-aqua shadow-[0_12px_34px_rgba(168,72,115,0.12)]"
                  aria-hidden="true"
                />
              </div>
  ...
  return (
    ...
    >
      ...
      <div className="section-shell">
        {isDesktop ? <DesktopVideoExamples /> : <MobileSafeVideoExamples />}
      </div>
    </section>
  );
}
